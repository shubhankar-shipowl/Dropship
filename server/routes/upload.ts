...
export function registerUploadRoutes(app: Express): void {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      console.log(
        `[UPLOAD START] File upload started from ${clientIP} at ${new Date().toISOString()}`,
      );
      ...
      try {
        ...
        if (
          ...
          data = XLSX.utils.sheet_to_json(worksheet, {
            ...
            raw: true, // Keep raw values to access cell properties
          });
        } else if (mimetype.includes('csv') || originalname.endsWith('.csv')) {
          const csvData: any[] = [];
          const stream = Readable.from(buffer.toString());
        ...
        const missingFields = requiredFields.filter(
          (field) => !(field in columnMapping) || columnMapping[field] === -1,
        );
        ...
        try {
          ...
          console.log(
            `Created upload session: ${uploadSession.id} for ${
              data.length - 1
            } rows`,
          );
        } catch (sessionError) {
        ...
        for (
          ...
          const now = Date.now();
          if (chunkStart === 1 || now - lastProgressLog > 30000) {
            ...
            console.log(
              `[UPLOAD PROGRESS] ${progress}% (${
                chunkStart - 1
              }/${totalRows} rows) in ${elapsed}s from ${clientIP}`,
            );
            lastProgressLog = now;
          }
          ...
          for (let i = chunkStart; i < chunkEnd; i++) {
            ...
            try {
              ...
              const dropshipperEmail =
                String(row[columnMapping.dropshipperEmail] || '').trim() || '';
              const orderId =
                String(row[columnMapping.orderId] || '').trim() || '';
              const productName =
                String(row[columnMapping.productName] || '').trim() || '';
              const sku = row[columnMapping.sku]
                ? String(row[columnMapping.sku]).trim()
                : null;
              const qty = parseInt(String(row[columnMapping.qty] || '0')) || 0;

              // Parse Product Value exactly as in Excel
              const productValueStr = String(
                row[columnMapping.productValue] || '',
              ).trim();
              let productValue = 0;
              ...
              const shippingProvider =
                String(row[columnMapping.shippingProvider] || '').trim() || '';
              const mode = columnMapping.mode
                ? String(row[columnMapping.mode] || '').trim() || null
                : null;

              // Handle dates
              const orderDate =
                parseDate(String(row[columnMapping.orderDate] || '')) ||
                new Date();
              const deliveredDate = columnMapping.deliveredDate
                ? parseDate(String(row[columnMapping.deliveredDate] || ''))
                : null;
              const rtsDate = columnMapping.rtsDate
                ? parseDate(String(row[columnMapping.rtsDate] || ''))
                : null;

              // Extract waybill directly from Excel cell to preserve exact value - NO CHANGES
              let waybill: string | null = null;
              if (columnMapping.waybill !== undefined) {
                if (worksheet) {
                  // Get cell directly from worksheet to preserve exact value
                  const cellAddress = XLSX.utils.encode_cell({
                    r: i,
                    c: columnMapping.waybill,
                  });
                  const cell = worksheet[cellAddress];

                  if (cell) {
                    // Priority 1: If cell is text type, use the exact string value (no conversion)
                    if (cell.t === 's' && cell.v) {
                      waybill = String(cell.v).trim() || null;
                    }
                    // Priority 4: Use raw value
                ...
                // Fallback: use value from parsed row data
                if (!waybill) {
                  const waybillValue = row[columnMapping.waybill];
                  if (
                    waybillValue !== undefined &&
                    waybillValue !== null &&
                    waybillValue !== ''
                  ) {
                    waybill = String(waybillValue).trim() || null;
                  }
                }
              }

              const productUid = generateProductUid(
                sku,
                productName,
                dropshipperEmail,
              );
        ...
        console.log(
          `Total processed: ${processedCount} orders, cancelled: ${cancelledCount}`,
        );
        ...
        console.log(
          `[UPLOAD SUCCESS] Completed in ${uploadDuration}s - ${processedCount} rows processed from ${clientIP}`,
        );
        ...
        console.error(
          `[UPLOAD ERROR] Failed after ${uploadDuration}s from ${clientIP}:`,
          error,
        );
        console.error(
          'Error stack:',
