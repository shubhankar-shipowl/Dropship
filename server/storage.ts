...
export class DatabaseStorage implements IStorage {
      ...
      for (const order of orderItems) {
        ...
        if (isDelivered && inDeliveredDateRange) {
          ...
          if (isCodOrder) {
            ...
            codReceived = productValuePerUnit; // qty × individual product value per row
  ...
  async getCodBreakdownForRange(params: {
    ...
    console.log(
      `🔍 COD Breakdown Query: ${deliveredDateFrom} to ${deliveredDateTo} for ${
      ...
      if (isCodOrder) {
        for (const order of orderItems) {
          ...
          console.log(
            `💰 COD Order ${
              order.orderId
            }: ${order.deliveredDate?.toDateString()} | Mode: ${paymentMode} | Product Value: ₹${
        ...
        console.log(
          `⚠️ Non-COD Order ${orderId}: Mode: ${paymentMode} - Skipped`,
        );
      }
    }

    console.log(
      `💯 Total COD for range: ₹${Math.round(totalCod * 100) / 100} from ${
        orderDetails.length
      } order items`,
    );
    ...
    try {
      ...
      for (const [orderId, orderItems] of Array.from(orderGroups.entries())) {
        ...
        const totalCodAmount =
          parseFloat(rawCodAmount.replace(/[₹,\s]/g, '')) || 0;
        const codPerUnit =
  ...
  async getDropshipperPayoutSummary(
    ...
    try {
      console.log(
        `📊 Getting dropshipper summary with dates: ${dateFrom} to ${dateTo}`,
      );
      let dateFilter = '';
      ...
      for (const currentOrder of currentRtsRtoOrders) {
        ...
        for (let i = 0; i < orderHistory.length - 1; i++) {
          ...
          if (
            ...
            console.log(
              `Status change detected for ${currentOrder.orderId}: ${prevOrder.status} → ${nextOrder.status}`,
            );
            break;
          }
        }
